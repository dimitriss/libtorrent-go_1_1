    ...
    public:
        ...
        int read(char *read_buf, int size, int piece, int offset) {
            ...
            if (pieces[piece].size < pieces[piece].length) {
                if (is_logging) {
                    std::cerr << "INFO less: " << piece << ", off: " << offset << ", size: " << pieces[piece].size
        ...
        int readv(libtorrent::file::iovec_t const *bufs, int num_bufs, int piece, int offset, int flags,
            ...
            if (is_logging) {
                ...
                if (is_logging) {
                    std::cerr << "INFO readv out p: " << piece << ", pl: " << pieces[piece].length
                              ...
                              << ", off: " << offset
                              << ", bs: " << buffers[pieces[piece].bi].buffer.size() << ", res: " << size << "=" << n
                              << std::endl;
                }
            };
        ...
        int writev(libtorrent::file::iovec_t const *bufs, int num_bufs, int piece, int offset, int flags,
                   libtorrent::storage_error &ec) {
            if (is_logging) {
                std::cerr << "INFO writev in  p: " << piece << ", off: " << offset << ", bufs: "
                          << bufs_size(bufs, num_bufs) << std::endl;
            };
        ...
        bool verify_resume_data(libtorrent::bdecode_node const &rd, std::vector <std::string> const *links,
        ...
        void trim(int pi) {
            ...
            while (buffer_used >= buffer_limit) {
                if (is_logging) {
                    std::cerr << "INFO Trimming " << buffer_used << " to " << buffer_limit << " with reserved "
                              << buffer_reserved << ", " << get_buffer_info() << std::endl;
                }
                ...
                if (!reader_pieces.empty()) {
                    ...
                    if (bi != -1) {
                        if (is_logging) {
                            std::cerr << "INFO Removing non-read piece: " << buffers[bi].pi << ", buffer:" << bi
                                      << std::endl;
                        }
                        remove_piece(bi);
                        continue;
        ...
        std::string get_buffer_info() {
            ...
            for (int i = 0; i < buffers.size(); i++) {
                ...
                result += boost::lexical_cast<std::string>(buffers[i].index) + ":" +
                          boost::lexical_cast<std::string>(buffers[i].pi);
            };
